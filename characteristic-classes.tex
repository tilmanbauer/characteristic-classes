...
\begin{defn}
A \emph{smooth structure} on a space $M$ is a local sheaf $C^\infty$ of $\R$-algebras on $M$, called the \emph{structure sheaf}, such $(M,C^\infty)$ is locally isomorphic , as a locally ringed space, to $(\R^n,C^\infty)$ for some $n$. A \emph{smooth manifold} is a second-countable Hausdorff space together with a smooth structure.
...
Conversely, let $(\{U_\alpha\},\phi_\alpha)_{\alpha \in I}$ be a maximal smooth atlas on $M$. Using the gluing condition, we define a local sheaf of rings on $M$ as follows: $C^\infty_{U_\alpha} = (\phi_\alpha^{-1})_* C^\infty_{\R^{n_\alpha}}$. For an intersection $U_{\alpha\beta}$, we must show that 
\[
