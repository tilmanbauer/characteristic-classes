...
\tilde g(x,u) = (g(u)(x),u)
\]
for some function $g\colon U_{\alpha\beta} \to \map(k^{n_\alpha},k^{n_\beta})$. Furthermore, since $\tilde g$ is a linear isomorphism on fibers, we actually have that $g\colon U_{\alpha\beta} \to \GL_{n_\alpha}(k^{n_\alpha})$.
...
\begin{defn}
Let $\{G_n\}_{n \geq 0}$ be a sequence of topological groups and $\mathcal U = \{U_\alpha\}_{\alpha \in I}$ an open cover of some space $B$. Then a \emph{cocycle for $\mathcal U$ with values in $\{G_n\}$} consists of continuous maps
\[
n\colon I \to \N
\]
...
The assignment of a cocycle $g_{\alpha\beta}$ to a vector bundle $p$ is not well-defined as it depends both on a choice of trivializing neighborhoods and a choice of trivializations over the latter. The following proposition clarifies, among other things, what happens when we choose different trivializations.

\begin{prop}
Let $p\colon E \to B$ and $p'\colon E' \to B$ two vector bundles. Let $\{U_\alpha\}_\alpha$ be a cover of $B$ that trivializes both $p$ and $p'$, and let $g_{\alpha\beta}$ and $g'_{\alpha\beta}$ be cocycles associated to $p$ and $p'$, respectively. Then $p \cong p'$ iff and only $g$ and $g'$ are cohomologous.
\end{prop}
...
\]
for some collection $c_\alpha\colon U_\alpha \to G_{n_\alpha}$. For $\alpha \in I$, define a map
\[
E_{U_\alpha} \xrightarrow{f_\alpha} E'_{U_\alpha}, \quad f_\alpha = (\phi'_{\alpha})^{-1} \circ (c_\alpha \times \id_{U_\alpha}) \circ \phi_\alpha.
\]
...
\begin{align*}
f_\beta|_{U_{\alpha\beta}} \circ (f_\alpha|_{U_{\alpha\beta}})^{-1} =& (\phi'_\beta)^{-1} \circ (c_\beta \times \id) \circ \phi_{\beta} \circ (\phi_{\alpha})^{-1} \circ (c_\alpha^{-1} \times \id) \circ \phi'_\alpha\\
=& (\phi'_\beta)^{-1} \circ ((c_\beta g_{\alpha\beta}c_{\alpha}^{-1}) \times \id) \circ \phi'_\alpha\\
= & (\phi'_\beta)^{-1} \circ (g'_{\alpha\beta} \times \id) \circ \phi'_\alpha = \id,
\end{align*}
where all maps are understood as their restriction to $U_{\alpha\beta}$. Thus $f_\alpha|_{U_{\alpha\beta}} = f_beta|_{U_{\alpha\beta}}$ and thus we can glue these functions together to a globally defined isomorphism of vector bundles
\[
f\colon E \to E', \quad f(x) = f_\alpha(x) \quad \text{if } x \in U_\alpha.
\]

Now assume that $f\colon E \to E'$ is an isomorphism of vector bundles. Define $c_\alpha\colon U_\alpha \to G_{n_\alpha}$ by 
\[
c_\alpha \times \id_{U_\alpha} = \phi'_\alpha \circ f \circ \phi_\alpha^{-1}.
\]
Then
\begin{align*}
c_\beta g_{\alpha\beta} c_\alpha^{-1} \times \id = & \phi'_\beta \circ f \circ \phi_\beta^{-1} \circ \phi_\beta \circ \phi_\alpha^{-1} \circ \phi_\alpha \circ f^{-1} \circ (\phi'_\alpha)^{-1}\\
 = & \phi'_\beta (\phi'_{\alpha})^{-1} = g'_{\alpha\beta},
\end{align*}
hence $g$ and $g'$ are cohomologous.
\end{proof}

Thus for a chosen trivializing cover, the cocycle of a vector bundle is unique up to homology. The following result gives the existence part of the characterization:
...
A \emph{functor} $F$ between category objects $\C$ and $\D$ in $C$ consists of two functors
\[
...
A \emph{natural transformation} $\alpha\colon \C \to \D$ is a functor
\[
\alpha\colon \C \times [1] \to \D,
\]
where $[1]$ denotes the category with two objects $0$, $1$ and exactly one morphism $0 \to 1$ besides the identity morphisms. We say that the natural transformation goes from the functor $\alpha_0\colon \C \cong \C \times \{0\} \to \D$ to the functor $\alpha_1\colon \C \cong \C \times \{1\} \to \D$.
...
\[
\Fun(\mathbf U,\mathcal G) \to Z^1_{\mathcal U}(X,\mathcal G), \quad F \mapsto (n_\alpha,g_{\alpha\beta}) = (F_0|_{U_\alpha},F_1|_{U_{\alpha\beta}})
\]
is a bijection.
...
Now let $\alpha\colon \mathbf U \to \mathcal G$ be a natural transformation from a functor $F$ to a functor $F'$, and that $(n_\alpha,g_{\alpha\beta})$ and $(n'_\alpha,g'_{\alpha\beta})$ are the cocycles associated to $F$ and $F'$, respectively. Since for $x \in U_\alpha$, $(x,0 \to 1)$ is a morphism in $\mathbf U \times [1]$, there must be a morphism between $n_\alpha$ and $n'_\alpha$ in $\mathcal G$, so $n_\alpha=n'_\alpha$ for all $\alpha$.

Now define $c_\alpha\colon U_\alpha \to G_{n_\alpha}$ by $c_\alpha = \alpha|_{U_\alpha}$. We want to show that
\[
...
\]
This can be reformulated as $g'_{\alpha\beta}c_\alpha = c_\beta g_{\alpha\beta}$. This follows from the functoriality of $\alpha$ because the above identity is the image of the true identity
\[
...
Conversely, if $(n,g)$ and $(n',g')$ are cohomologous by $c_\alpha\colon U_\alpha \to G_{n_\alpha}$ then $\alpha(x \in U_\alpha,0) = \alpha(x \in U_\alpha,1) = n_\alpha=n'_\alpha$ defines the object part of a natural transformation. On morphisms of the form $(x \in U_{\alpha\alpha},0 \to 1)$, $\alpha = \coprod_{\alpha} c_\alpha \colon \mathbf U \to \mathcal G$.
\end{proof}

We will pursue the cocycle characterization of vector bundles further in Chapter~\ref{ch:cohomology} and also clarifying the terms “cocycle”, “coboundary”, and “homologous.”
...
\begin{defn}
Let $A$ be an $\R$-algebra. A \emph{derivation} on $A$ is a $k$-linear function $D\colon A \to A$ such that $D(ab) = D(a)b+D(b)a$. Denote the set of derivations on $A$ be $\Der(A)$; this is an $\R$-vector space.
\end{defn}
