...
\begin{defn}
Let $E$ be a smooth $k$-vector bundle on a smooth manifold $M$, considered as a sheaf of locally free $C^\infty_M$-modules. A \emph{connection} on $E$ is a map of $C^\infty_M$-module sheaves
\[
\nabla\colon E \to \Omega^1_M \otimes E
\]
...
\]
\end{defn}
Note that $\nabla$ is \emph{not} a map of vector bundles: it is not required to be a map of \emph{local} $C^\infty_M$-module sheaves. That is, $\nabla$ is not determined by its values on fibers alone, much as the derivative of a function at a point $x$ is not determined by the value $f(x)$ alone.
